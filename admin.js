...
function renderDashboard() {
  ...
  const analytics = DB.get(DB.KEYS.analytics);
  const today = new Date().toISOString().slice(0, 10);
  const todayViews = (analytics.byDate && analytics.byDate[today]) || 0;
  const counts = {
  ...
  if (!grid) return;
  grid.innerHTML = `
    <div class="stat-card glass"><div class="stat-num">${analytics.unique || 0}</div><div class="stat-label">Unikátních návštěvníků</div></div>
    <div class="stat-card glass"><div class="stat-num">${analytics.total || 0}</div><div class="stat-label">Zobrazení webu celkem</div></div>
    <div class="stat-card glass"><div class="stat-num">${todayViews}</div><div class="stat-label">Zobrazení dnes</div></div>
  ...
  `;
  renderCharts(analytics, leads, auditRequests);
}

/* ============================================================
   GRAFY (Chart.js) — trend návštěvnosti a poptávek za 14 dní
============================================================ */
...
function countByDate(items, days) {
...
function renderCharts(analytics, leads, auditRequests) {
  if (typeof Chart === "undefined") return; // Chart.js se nenačetl (např. výpadek CDN) — dashboard i tak funguje bez grafů
  const days = lastNDays(14);
  const labels = days.map(shortLabel);

  const visitsCanvas = document.querySelector("#visits-chart");
  if (visitsCanvas) {
    const data = days.map((d) => (analytics.byDate && analytics.byDate[d]) || 0);
    ...
    visitsChartInstance = new Chart(visitsCanvas, {
      ...
      data: {
        ...
        datasets: [{
          ...
          data,
          borderColor: "#2ee6c8",
          backgroundColor: "rgba(46,230,200,.15)",
          fill: true,
          tension: 0.35,
          pointRadius: 3,
          pointBackgroundColor: "#2ee6c8",
        }],
      },
  ...
  const leadsCanvas = document.querySelector("#leads-chart");
  if (leadsCanvas) {
    leadsChartInstance?.destroy();
    leadsChartInstance = new Chart(leadsCanvas, {
      type: "bar",
      data: {
        labels,
        datasets: [
          { label: "Poptávky", data: countByDate(leads, days), backgroundColor: "#7c5cff", borderRadius: 4 },
          { label: "Audit poptávky", data: countByDate(auditRequests, days), backgroundColor: "#2ee6c8", borderRadius: 4 },
        ],
      },
      options: chartOptions(),
    });
...
function initProcess() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (editIdx !== undefined && editIdx !== "") {
      items[+editIdx] = payload;
      delete form.dataset.editIdx;
      form.querySelector("button[type=submit]").textContent = "+ Přidat krok";
    } else {
      items.push(payload);
    }
...
function renderProcessAdmin() {
  ...
  container.querySelectorAll("[data-edit]").forEach((btn) => btn.addEventListener("click", () => {
    ...
    form.dataset.editIdx = btn.dataset.edit;
    form.querySelector("button[type=submit]").textContent = "Uložit úpravu";
...
function initFaq() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (editIdx !== undefined && editIdx !== "") {
      items[+editIdx] = payload;
      delete form.dataset.editIdx;
      form.querySelector("button[type=submit]").textContent = "+ Přidat otázku";
    } else {
      items.push(payload);
    }
...
function renderFaqAdmin() {
  ...
  container.querySelectorAll("[data-edit]").forEach((btn) => btn.addEventListener("click", () => {
    const it = items[+btn.dataset.edit];
    const form = document.querySelector("#faq-form");
    form.q.value = it.q || ""; form.q_en.value = it.q_en || "";
    form.a.value = it.a || ""; form.a_en.value = it.a_en || "";
    form.dataset.editIdx = btn.dataset.edit;
    form.querySelector("button[type=submit]").textContent = "Uložit úpravu";
    form.scrollIntoView({ behavior: "smooth", block: "center" });
  }));
...
function renderAuditList() {
  ...
  container.innerHTML = items.map((m) => {
    const s = m.scores;
    const scoreBadges = s ? `
      <div class="audit-admin-scores">
        <span title="Rychlost">⚡ ${s.performance}</span>
        <span title="SEO">🔍 ${s.seo}</span>
        <span title="Bezpečnost a postupy">🛡️ ${s.bestPractices}</span>
        <span title="Přístupnost">♿ ${s.accessibility}</span>
      </div>` : `<span class="tag">bez skóre (analýza selhala)</span>`;
    const issuesHtml = (m.issues && m.issues.length)
      ? `<ul class="audit-admin-issues">${m.issues.map((i) => `<li>${esc(i)}</li>`).join("")}</ul>` : "";
    const messageHtml = m.message
      ? `<textarea readonly rows="6" class="audit-admin-message">${esc(m.message)}</textarea>
         <button type="button" class="btn btn-ghost btn-sm" data-copy-msg="${m.id}">⧉ Kopírovat zprávu</button>` : "";
    return `
    <div class="item-row" style="align-items:flex-start; flex-direction:column; gap:12px;">
  ...
  }).join("") || `<div class="empty-hint">Zatím žádné audit poptávky.</div>`;
  ...
  container.querySelectorAll("[data-copy-msg]").forEach((btn) => btn.addEventListener("click", async () => {
    const item = items.find((m) => m.id === btn.dataset.copyMsg);
    try {
      await navigator.clipboard.writeText(item.message);
      toast("Zpráva zkopírována.");
    } catch {
      toast("Kopírování se nepodařilo — označ text ručně.");
    }
  }));
}
...
function initNotes() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (editId) {
      ...
      delete form.dataset.editId;
      form.querySelector("button[type=submit]").textContent = "Přidat poznámku";
    } else {
      const note = { id: DB.uid(), done: false, ...payload };
...
function renderNotes() {
  ...
  grid.querySelectorAll("[data-edit]").forEach((btn) => btn.addEventListener("click", () => {
    const n = items.find((x) => x.id === btn.dataset.edit);
    const form = document.querySelector("#notes-form");
    form.text.value = n.text || "";
    form.color.value = n.color || "#7c5cff";
    form.priority.value = n.priority || "med";
