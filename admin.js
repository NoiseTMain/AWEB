...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Pokud data někdo změní jinde (jiný admin, jiné zařízení), aspoň
  // dashboard se osvěží živě bez nutnosti obnovit stránku.
  window.addEventListener("pf:update", (e) => {
    if (e.detail?.remote && sessionStorage.getItem("pf_admin_auth") === "1") {
      renderDashboard();
      if (e.detail.key === DB.KEYS.contactMessages) renderMessages();
    }
  });
});
