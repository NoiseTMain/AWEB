...
- **Skutečné logo v Hero** — místo 3D otočného monogramu je teď v Hero
  sekci vidět stejné logo jako v navigaci (statické, žádné pohyblivé 3D).
  Soubory `logo3d.js` a knihovny Three.js/FontLoader/OrbitControls byly
  odstraněny — nepoužívaly se a zbytečně zatěžovaly načítání.
- **Návštěvnost webu + grafy** — na Přehledu v adminu vidíš unikátní
  návštěvníky, celková zobrazení, zobrazení dnes, a k tomu **graf**
  vývoje za posledních 14 dní (Chart.js) a graf poptávek vs. audit
  poptávek za stejné období. Anonymní, žádné osobní údaje.
- **Kategorizovaná navigace v adminu** — postranní menu je rozdělené do
  skupin (Obsah webu / Marketing a obchod / Organizace / Ostatní),
  navíc teď jde uvnitř menu scrollovat a tlačítko Odhlásit se je vždy
  vidět dole, nezávisle na scrollu.
- **Vypnuté přiblížení (zoom) na mobilu** — na webu i v administraci,
  opravuje to i vizuální artefakty (černé pruhy nahoře/dole), které
  přiblížení na iOS Safari občas způsobovalo u pevně ukotveného pozadí.
- **Editace u Poznámek, FAQ a Procesu** — dřív šlo jen přidávat a mazat,
  teď má každá položka i tlačítko úpravy (stejně jako Projekty/Reference/
  Služby/Odkazy).
- **Reálný audit webu přes Google PageSpeed Insights** — po odeslání URL
  proběhne skutečná analýza (rychlost, SEO, přístupnost, bezpečnostní
  postupy) veřejným Google API, bez potřeby vlastního backendu nebo
  tajného klíče. Výsledky se zobrazí rovnou na stránce (ne e-mailem —
  to bychom bez backendu nedokázali odeslat) a rovnou se vygeneruje
  návrh zprávy pro klienta s konkrétními zjištěními, kterou vidíš i v
  administraci u každé audit poptávky (s tlačítkem na zkopírování).
  Pozor: toto je automatizovaná technická analýza (Google Lighthouse),
  ne generativní AI — text zprávy je poskládaný z reálných dat podle
  šablony, nejde o jazykový model. Skutečné AI generování textu (např.
  přes Claude/OpenAI) by vyžadovalo vlastní server s bezpečně uloženým
  API klíčem — to nejde nasadit jen nahráním souborů na GitHub, chce to
  nasazení přes příkazovou řádku z počítače. Klidně to pak doplním.

- **Návštěvnost webu** — v adminu na Přehledu vidíš unikátní návštěvníky
  (podle prohlížeče), celkový počet zobrazení a zobrazení za dnešek.
  Anonymní, žádné osobní údaje — jen souhrnná čísla, žádné cookies ani
